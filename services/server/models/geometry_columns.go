...
func AddGeometryColumnHook(hookPoint boil.HookPoint, geometryColumnHook GeometryColumnHook) {
	...
	case boil.AfterSelectHook:
		geometryColumnAfterSelectMu.Lock()
		geometryColumnAfterSelectHooks = append(geometryColumnAfterSelectHooks, geometryColumnHook)
		geometryColumnAfterSelectMu.Unlock()
	case boil.BeforeInsertHook:
		geometryColumnBeforeInsertMu.Lock()
		geometryColumnBeforeInsertHooks = append(geometryColumnBeforeInsertHooks, geometryColumnHook)
		geometryColumnBeforeInsertMu.Unlock()
	case boil.AfterInsertHook:
		geometryColumnAfterInsertMu.Lock()
		geometryColumnAfterInsertHooks = append(geometryColumnAfterInsertHooks, geometryColumnHook)
		geometryColumnAfterInsertMu.Unlock()
	case boil.BeforeUpsertHook:
		geometryColumnBeforeUpsertMu.Lock()
		geometryColumnBeforeUpsertHooks = append(geometryColumnBeforeUpsertHooks, geometryColumnHook)
		geometryColumnBeforeUpsertMu.Unlock()
	case boil.AfterUpsertHook:
		geometryColumnAfterUpsertMu.Lock()
		geometryColumnAfterUpsertHooks = append(geometryColumnAfterUpsertHooks, geometryColumnHook)
		geometryColumnAfterUpsertMu.Unlock()
	}
}

// OneG returns a single geometryColumn record from the query using the global executor.
func (q geometryColumnQuery) OneG(ctx context.Context) (*GeometryColumn, error) {
...
// AllG returns all GeometryColumn records from the query using the global executor.
func (q geometryColumnQuery) AllG(ctx context.Context) (GeometryColumnSlice, error) {
...
// CountG returns the count of all GeometryColumn records in the query using the global executor
func (q geometryColumnQuery) CountG(ctx context.Context) (int64, error) {
...
func (q geometryColumnQuery) ExistsG(ctx context.Context) (bool, error) {
...
func (o *GeometryColumn) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(geometryColumnAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(geometryColumnPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert geometry_columns, could not build conflict column list")
			}

			conflict = make([]string, len(geometryColumnPrimaryKeyColumns))
			copy(conflict, geometryColumnPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"geometry_columns\"", updateOnConflict, ret, update, conflict, insert, opts...)
